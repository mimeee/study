...
class BaiduInjector extends Injector {
    ...
    constructor() {
        super();
        sessionStorage.setItem('baiduPage', JSON.stringify([]))
        sessionStorage.setItem('baiduUpdateList', JSON.stringify([]))
    ...
    _nextPageOnClick() {
        ...
        const timer = setInterval(function(){
            if($('.new-pagenav-right a')[0] || $('.new-nextpage-only')[0]){
                if($('.new-pagenav-right a')[0])$('.new-pagenav-right a')[0].click();
                if($('.new-nextpage-only')[0])$('.new-nextpage-only')[0].click();
                clearInterval(timer);
            }

            if(!$('.new-pagenav-right a')[0] && $('.new-pagenav-left a')[0]){
                $('[data-tag="listLoadding"]').addClass('h-hide');
                $('[data-tag="keepLoadding"]').addClass('h-hide');
                $('[data-tag="listEnd"]').removeClass('h-hide');
                clearInterval(timer);
                // that.$bar.listLoadingTextTroggle('listEnd');
            }
        }, 0);
    }
