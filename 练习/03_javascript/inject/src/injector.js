...
class Injector {
    ...
    constructor() {
        ...
        //window.WAP_HOST_LIST
        this.wapHostList = map(window.WAP_HOST_LIST || hostList || [], host => new RegExp(host.replace(/\./g, '\\.')));
        console.log(this.wapHostList);
        ...
        this.$bar.touchend(this._nextPageOnClick);
    ...
    inject() {
        ...
        console.log('inject函数');
        let legalUpdateList = [];
        let updateList = [];
        let resultElementList = this._queryResultElementList();
        console.log('resultElementList', resultElementList);
        ...
        console.log('end forEach resultElementList');
        ...
        if (this.isFristIneject) {
            setTimeout(() => {
                {
                   this.$bar.noResultClick(()=>this.autoTurnPage());
        ...
        if(sessionStorage.getItem('baiduPage')){
            var page =  JSON.parse(sessionStorage.getItem('baiduPage'));
            page.push(this.currentPage);
            sessionStorage.setItem('baiduPage', JSON.stringify(page));
        ...
        if (!this._callback) {
            ...
            if ('MutationObserver' in window) {
                ...
                try{
                    ...
                    this.$bar.secondShrink('未搜索到结果，试试其他搜索引擎');
                    console.log(e)
                }
